...
async function proxyRequestToAxios(workflow, additionalData, node, uriOrObject, options) {
    ...
    try {
        ...
        if (configObject.resolveWithFullResponse === true) {
            ...
            await ((_b = additionalData === null || additionalData === void 0 ? void 0 : additionalData.hooks) === null || _b === void 0 ? void 0 : _b.executeHookFunctions('nodeFetchedData', [workflow === null || workflow === void 0 ? void 0 : workflow.id, node]));
            return {
                body,
            ...
            await ((_c = additionalData === null || additionalData === void 0 ? void 0 : additionalData.hooks) === null || _c === void 0 ? void 0 : _c.executeHookFunctions('nodeFetchedData', [workflow === null || workflow === void 0 ? void 0 : workflow.id, node]));
            return body;
        }
    }
    catch (error) {
        ...
        if (error.isAxiosError) {
            if (response) {
                ...
                throw Object.assign(error, {
                    message,
                    statusCode: response.status,
                    options: (0, pick_1.default)(config !== null && config !== void 0 ? config : {}, ['url', 'method', 'data', 'headers']),
                    error: responseData,
                    config: undefined,
                    request: undefined,
...
async function httpRequestWithAuthentication(credentialsType, requestOptions, workflow, node, additionalData, additionalCredentialOptions) {
    ...
    try {
        ...
        if (credentialsDecrypted === undefined) {
            throw new n8n_workflow_1.NodeOperationError(node, `Node "${node.name}" does not have any credentials of type "${credentialsType}" set!`, { severity: 'warning' });
        }
...
async function getCredentials(workflow, node, type, additionalData, mode, runExecutionData, runIndex, connectionInputData, itemIndex) {
    ...
    if (!fullAccess) {
        if (nodeType.description.credentials === undefined) {
            throw new n8n_workflow_1.NodeOperationError(node, `Node type "${node.type}" does not have any credentials defined!`, { severity: 'warning' });
        }
        ...
        if (nodeCredentialDescription === undefined) {
            throw new n8n_workflow_1.NodeOperationError(node, `Node type "${node.type}" does not have any credentials of type "${type}" defined!`, { severity: 'warning' });
        }
    ...
    if (!fullAccess && !((_a = node.credentials) === null || _a === void 0 ? void 0 : _a[type])) {
        if ((nodeCredentialDescription === null || nodeCredentialDescription === void 0 ? void 0 : nodeCredentialDescription.required) === true) {
            ...
            if (!node.credentials[type]) {
                throw new n8n_workflow_1.NodeOperationError(node, `Node does not have any credentials set for "${type}"!`, { severity: 'warning' });
            }
        }
        else {
...
const validateResourceMapperValue = (parameterName, paramValues, node, skipRequiredCheck = false) => {
    ...
    for (let i = 0; i < paramValueNames.length; i++) {
        ...
        if (schemaEntry === null || schemaEntry === void 0 ? void 0 : schemaEntry.type) {
            const validationResult = (0, n8n_workflow_1.validateFieldType)(key, resolvedValue, schemaEntry.type, schemaEntry.options);
...
const validateValueAgainstSchema = (node, nodeType, inputValues, parameterName, runIndex, itemIndex) => {
    var _a, _b;
    let validationResult = { valid: true, newValue: inputValues };
    const resourceMapperField = nodeType.description.properties.find((prop) => n8n_workflow_1.NodeHelpers.displayParameter(node.parameters, prop, node) &&
        prop.type === 'resourceMapper' &&
        parameterName === `${prop.name}.value`);
    if (resourceMapperField && typeof inputValues === 'object') {
        validationResult = validateResourceMapperValue(parameterName, inputValues, node, ((_b = (_a = resourceMapperField.typeOptions) === null || _a === void 0 ? void 0 : _a.resourceMapper) === null || _b === void 0 ? void 0 : _b.mode) !== 'add');
    }
    if (!validationResult.valid) {
        throw new n8n_workflow_1.ExpressionError(`Invalid input for '${String(validationResult.fieldName) || parameterName}' [item ${itemIndex}]`, {
...
function getNodeParameter(workflow, runExecutionData, runIndex, connectionInputData, node, parameterName, itemIndex, mode, timezone, additionalKeys, executeData, fallbackValue, options) {
    ...
    returnData = validateValueAgainstSchema(node, nodeType, returnData, parameterName, runIndex, itemIndex);
    return returnData;
}
