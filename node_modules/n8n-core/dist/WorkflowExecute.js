...
class WorkflowExecute {
    ...
    forceInputNodeExecution(workflow, node) {
        ...
        if (forceInputNodeExecution !== undefined) {
            if (typeof forceInputNodeExecution === 'string') {
                forceInputNodeExecution = !!workflow.expression.getSimpleParameterValue(node, forceInputNodeExecution, this.mode, this.additionalData.timezone, { $version: node.typeVersion });
            }
                ...
                for (const connections of incomingNodeConnections.main) {
                    for (let inputIndex = 0; inputIndex < connections.length; inputIndex++) {
                        ...
                        if (node && pinData && pinData[node.name]) {
                            ...
                            const nodeIncomingData = (_b = (_a = runData[connection.node][runIndex]) === null || _a === void 0 ? void 0 : _a.data) === null || _b === void 0 ? void 0 : _b[connection.type][connection.index];
                            if (nodeIncomingData) {
    ...
    incomingConnectionIsEmpty(runData, inputConnections, runIndex) {
        for (const inputConnection of inputConnections) {
            const nodeIncomingData = (0, get_1.default)(runData, `[${inputConnection.node}][${runIndex}].data.main[${inputConnection.index}]`);
                    ...
                    for (let tryIndex = 0; tryIndex < maxTries; tryIndex++) {
                        ...
                        try {
                            ...
                            if (nodeSuccessData === null || nodeSuccessData === void 0 ? void 0 : nodeSuccessData.length) {
                                const isSingleInputAndOutput = executionData.data.main.length === 1 && ((_a = executionData.data.main[0]) === null || _a === void 0 ? void 0 : _a.length) === 1;
                        ...
                        if (workflow.connectionsBySourceNode[executionNode.name].hasOwnProperty('main')) {
                            ...
                            for (outputIndex in workflow.connectionsBySourceNode[executionNode.name].main) {
                                ...
                                for (connectionData of workflow.connectionsBySourceNode[executionNode.name].main[outputIndex]) {
                                    ...
                                    const forceInputNodeExecution = this.forceInputNodeExecution(workflow, connectionDestinationNode);
                        ...
                        for (let i = 0; i < waitingNodes.length; i++) {
                            ...
                            if (requiredInputs !== undefined) {
                                if (typeof requiredInputs === 'string') {
                                    requiredInputs = workflow.expression.getSimpleParameterValue(checkNode, requiredInputs, this.mode, this.additionalData.timezone, { $version: checkNode.typeVersion }, undefined, []);
                                }
                                if ((requiredInputs !== undefined &&
